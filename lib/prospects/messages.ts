...
const byGroup: Record<Niche["group"], { details: string; impact: string; conversion: string }> = {
  Saúde: {
    ...
    impact: "acaba decidindo só pelo Google e pelo WhatsApp, sem ver especialidades, equipe e como agendar",
    conversion: "agendamentos",
  },
  "Serviços locais": {
    details: "serviços, área atendida e trabalhos feitos",
    impact: "acaba comparando só pelo preço no WhatsApp, sem ver serviços, área atendida e trabalhos feitos",
...
export function buildMessage(p: Prospect, step: CadenceStep, sender: string) {
  ...
  switch (step.key) {
    case "d0":
      return (
        `Olá, ${c.name}! Sou ${sender}, da Nexcript. Pesquisando ${c.term} em ${c.place}, ${observation(p)}. ` +
    ...
    case "d5": {
      const insight = hasKeywordName(p.name)
